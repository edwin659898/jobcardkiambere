                                                    ...
                                                    <form>
                                                        ...
                                                        <div class="flex flex-col rounded-md shadow-lg px-3 py-2 pt-3">
                                                            ...
                                                            <div v-if="SelectedOne"
                                                                ...
                                                                <button v-if="form.type === 'issue'" @click.prevent="issue()" :class="{ 'opacity-25': form.processing }" :disabled="form.processing"
                                                                ...
                                                                <button v-else @click.prevent="receive()"
                                                                    class="inline-flex justify-center rounded-md border border-transparent px-3 py-1 bg-blue-600 text-base leading-6 font-medium text-white shadow-sm hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                                                                    Receive
                                                                </button>
                                                            </div>
                                                                        ...
                                                                        <td>
                                                                            <ol v-for="stock, index in fruit.stocks" :key="index">
                                                                                    <li class="flex justify-between items-center space-x-3 mt-2">
                                                                                        <p>Issued: {{ stock.quantity }} Received: {{ stock.damage_seed }}
                                                                                        <!-- <p>{{ format_date(stock.created_at) }}</p> -->
                                                                                        <i v-if="stock.damage_seed === null" @click.prevent="selected(stock.id,'receive')"
...
export default {
    ...
    methods: {
        issue() {
            this.form.patch(`/Depulping/stock-issue/${this.form.SelectedId}`, { 
                onSuccess: () => this.SelectedOne = false,
                preserveScroll: true 
            });
        },
        receive() {
            this.form.patch(`/Depulping/receive-quantity/${this.form.SelectedId}`, { 
                onSuccess: () => this.SelectedOne = false,
                preserveScroll: true 
            });
        },
